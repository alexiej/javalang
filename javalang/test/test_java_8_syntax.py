...
interface Foo {
    ...
    def test_method_start_end(self):
        method_source = """void methodA() {
...
""" % (
        ...
        root = parse.parse(code)
        class_declaration = root.types[0]
        method_declararion = class_declaration.body[0]
        self.assertEqual(method_declararion.name, "methodA")

        code_lines = code.splitlines()
        source_code = get_source(method_declararion, code_lines)

        self.assertEqual(method_declararion.position.line, 3)
        self.assertEqual(method_declararion.position.column, 5)

        self.assertEqual(method_declararion.end_position.line, 5)
        self.assertEqual(method_declararion.end_position.column, 5)

        self.assertEqual(source_code, method_source)
