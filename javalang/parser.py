...
class Parser(object):
    ...
    def parse_member_declaration(self):
        ...
        prev = self.tokens.look(-1)
        if prev:
            member._end_position = prev.position
    ...
    def parse_method_or_field_declaraction(self):
        ...
        if self.tokens.value:
            member._end_position = self.tokens.value.position
    ...
    def parse_generic_method_or_constructor_declaration(self):
        ...
        prev = self.tokens.look(-1)
        if prev:
            method._end_position = prev.position
    ...
    def parse_block(self):
        ...
        while not self.would_accept("}"):
            ...
            prev = self.tokens.look(-1)
            if prev:
                statement._end_position = prev.position
        self.accept("}")
    ...
    def parse_statement(self):
        ...
        if self.would_accept("{"):
            ...
            if self.would_accept("{"):
                ...
                if catches is None and finally_block is None:
    ...
    def parse_primary(self):
        ...
        if isinstance(token, Literal):
            ...
            identifier_suffix._end_position = self.tokens.look(-1).position
    ...
    def parse_par_expression(self):
        self.accept("(")
        token = self.tokens.look(-1)
        expression = self.parse_expression()
        expression._position = token.position
        self.accept(")")
        expression._end_position = self.tokens.look(-1).position
